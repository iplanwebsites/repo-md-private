...
import repoBlogOfficial from "./repoBlogOfficial.js";
...
// "https://em-content.zobj.net/source/apple/419/grinning-face_1f600.png";
...
function augmentPost(a) {
  const author = a.frontmatter?.author || DEFAULT_AUTHOR;
  const defaultCover =
...
*/
  const post = await repoBlogOfficial.getPostBySlug(slug);
  // Return augmented version of the blog
  const aug = augmentPost(post);
  /*
  aug.similarBlogs = repoBlogOfficial.getSimilarPostsByHash(
    aug.hash,
    NB_SIMILAR_BLOGS
  );*/
...
export async function getBlogs() {
  const allPosts = await repoBlogOfficial.getAllPosts();
  const filtered = allPosts.filter((b) => !b.secret);

  const augmented = filtered.map(augmentPost);

  // we omit html prop
  const cleanner = augmented.map((b) => {
