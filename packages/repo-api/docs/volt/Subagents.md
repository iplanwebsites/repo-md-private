...
Supervisor Prompt Enhancement: The supervisor agent's system prompt is automatically modified (generateSupervisorSystemMessage) to include instructions on how to manage its subagents effectively. It lists the available subagents and their purpose and provides guidelines for delegation, communication, and response aggregation.
delegate_task Tool: A delegate_task tool is automatically added to the supervisor agent's available tools. This tool allows the supervisor LLM to decide when and how to delegate tasks.
Agent Registry: The parent-child relationship between the supervisor and its subagents is registered in the AgentRegistry, enabling discoverability and management within the broader system.
How Subagents Work
...
You can use hooks, specifically onHandoff, to inject custom logic when a task is delegated from a supervisor to a subagent via the delegate_task tool.
...
Check Agent Names: The delegate_task tool uses agent names, not IDs:
...
Improve Supervisor Instructions: Be explicit about when to delegate:

const supervisor = new Agent({
instructions: `
You coordinate specialized agents: - For creative writing: use Story Agent - For translation: use Translator Agent
...
Debug Context Passing: Check if context is being inherited:
