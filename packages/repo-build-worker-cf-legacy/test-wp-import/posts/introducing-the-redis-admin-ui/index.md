---
title: "Introducing the Redis Admin UI"
date: 2010-08-26
---

[![](images/redis.png "redis.png")](http://ww2.servicestack.net/wp-uploads/2010/03/redis.png)Confident that I've optimized ServiceStack's JSON web services performance enough with the adoption of my latest efforts developing [.NET's fastest JSON Serializer](http://www.servicestack.net/mythz_blog/?p=344), I'm now turning my attention towards creating apps that take advantage of it.
...
[![](images/nosql_thumb.gif "nosql_thumb.gif")](http://ww2.servicestack.net/wp-uploads/2010/03/nosql_thumb.gif)

### Redis, Sweet Redis

One of the exciting movements that have occurred in recent times is the introduction of NoSQL suite of data persistence solutions. There are numerous impressive NoSQL solutions out there but the one that I have been most interested in is **Redis** which from the [projects website](http://code.google.com/p/redis/):
...
I found this fascinating since it provides an extremely fast data-store (that gets routinely persisted) supporting rich data-structures that can be safely accessed by multiple app servers concurrently since all operations are atomic. Sweet just what I always wanted - although to make it productive I developed a [C# Redis Client](https://github.com/mythz/ServiceStack.Redis) that apart from supporting Redis's entire feature-set also provides a [high-level typed API](https://github.com/mythz/ServiceStack.Redis/wiki/IRedisTypedClient) that can persist any [.NET POCO Type](https://github.com/mythz/ServiceStack.Redis/wiki/DesigningNoSqlDatabase) which gets persisted as JSON in Redis.

## The Redis Admin UI

One of the disadvantages that comes with making use of a shiny new tech is that there is sometimes not a lot of tooling available for it. Despite its [vibrant community](http://groups.google.com/group/redis-db) this is also true for Redis where although it sports a rich command-line interface (Unix software is good like this) the GUI admin tools are somewhat lacking. Not to worry, I actually needed a project to work on to learn about [Google's closure-library](http://code.google.com/closure/library/) anyway so this ended up being a pretty good fit.

### Screenshots

Before we get into more detail its probably a good idea to showcase some of screenshots of where its currently at: Note: You can also try it out live: [http://www.servicestack.net/RedisAdminUI/AjaxClient/](http://www.servicestack.net/RedisAdminUI/AjaxClient/)

\[caption id="attachment\_399" align="alignnone" width="502" caption="Admin tab showing redis instance info"\][![](images/Admin_Tab.png "Admin Tab")](http://ww2.servicestack.net/wp-uploads/2010/08/Admin_Tab.png)\[/caption\]

\[caption id="attachment\_400" align="alignnone" width="502" caption="Aggregate view of complex types"\][![](images/View_Key_Group.png "View Key Group")](http://ww2.servicestack.net/wp-uploads/2010/08/View_Key_Group.png)\[/caption\]

\[caption id="attachment\_401" align="alignnone" width="502" caption="View single complex type"\][![](images/View_Complex_Type.png "View Complex Type")](http://ww2.servicestack.net/wp-uploads/2010/08/View_Complex_Type.png)\[/caption\]

### Redis Web Services

In order to be able to access Redis from a web page some JSON web services are in order. I could've just implemented the services required by the Admin UI although I wanted to flex some [ServiceStack](http://www.servicestack.net) muscle so decided to create web services for all of Redis's operations which on final count totalled near **100** web services that I ended up knocking out over a single weekend. One of the benefits of using ServiceStack to develop your web services is that you get SOAP, XML, JSON and JSV endpoints for free. So after spending the next couple of days creating unit tests to provide 100% coverage, the back-end was complete - thus giving Redis CouchDB-like powers by allowing it to be accessed from any HTTP client.
...
[http://www.servicestack.net/RedisAdminUI/Public/Metadata](http://www.servicestack.net/RedisAdminUI/Public/Metadata)

And some examples on how to call them:

[http://www.servicestack.net/RedisAdminUI/Public/Json/SyncReply/GetServerInfo](http://www.servicestack.net/RedisAdminUI/Public/Json/SyncReply/GetServerInfo) (JSON)

[http://www.servicestack.net/RedisAdminUI/Public/Xml/SyncReply/SearchKeys?Pattern=urn:c\*](http://www.servicestack.net/RedisAdminUI/Public/Xml/SyncReply/SearchKeys?Pattern=urn:c*) (XML)
...
In order to provide a useful Generic UI I've had to make a few assumptions on conventions used. Coincidentally these also happen to be the same conventions that the [ServiceStack's C# Redis Client](https://github.com/mythz/ServiceStack.Redis) uses when storing data :-).
...
Like the rest of ServiceStack the Redis Admin UI is Open source released under the liberal [new BSD licence](http://www.opensource.org/licenses/bsd-license.php).

In keeping with tradition with most of my software, the Redis Admin UI works cross-platform on Windows with .NET and Linux and OSX using [Mono](http://www.mono-project.com/Main_Page) (Live demo is hosted on CentOS/Nginx). I've had an attempt at some basic installation instructions that are included in the downloaded and [viewable online](https://github.com/mythz/ServiceStack.RedisWebServices/blob/master/src/RedisWebServices.Host/INSTALL.txt).

The latest version is hosted on Service Stacks code project site at the following url:

[**https://github.com/downloads/mythz/ServiceStack.RedisWebServices/RedisAdminUI.zip**](https://github.com/downloads/mythz/ServiceStack.RedisWebServices/RedisAdminUI.zip)

The Admin UI is highly customizable and very hackable since its written all in Java Script, so if you are interested in customizing the UI for your own purposes I invite you get started by downloading the [development version](https://github.com/mythz/ServiceStack.RedisWebServices) from svn trunk.
