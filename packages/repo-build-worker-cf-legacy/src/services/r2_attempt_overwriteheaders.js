...
// Upload file to the bucket (skip if exists)
export async function uploadFile(filePath, objectKey, options = {}) {
  ...
  try {
    ...
    try {
      ...
      // Create command with conditional header to skip if exists
      const command = new PutObjectCommand({
        ...
        Metadata: metadata,
        // Add conditional header if not overwriting
        ...(!options.overwrite && {
          IfNoneMatch: "*", // Only upload if the object doesn't exist
        }),
      });
      ...
      try {
        ...
        // Check if this was a precondition failed error (object exists)
...
// Upload content to the bucket (skip if exists)
...
// Upload stream to the bucket (skip if exists)
export async function uploadStream(stream, objectKey, options = {}) {
  ...
  try {
    ...
    return await upload(buffer, objectKey, { ...options, overwrite: true });
  } catch (error) {
