...
`;
      ...
      },
      {
        ...
        completion: async () => ['rss', 'wordpress', 'website', 'notion', 'roam', 'bear', 'evernote', 'logseq'],
  ...
  /migrate logseq <export>      - Import Logseq pages`,
          ...
          const migrationTypes = ['rss', 'wordpress', 'website', 'notion', 'roam', 'bear', 'evernote', 'logseq'];
          
          if (!migrationTypes.includes(subCommand)) {
            addMessage({
              type: MessageType.ERROR,
              content: `Unknown migration type: ${subCommand}. Available: ${migrationTypes.join(', ')}`,
              timestamp: new Date(),
            });
            return;
          }
          ...
          if (!args?.trim()) {
            addMessage({
              type: MessageType.ERROR,
              content: `Source path/URL required for ${subCommand} migration`,
              timestamp: new Date(),
            });
            return;
          }
          
          // Map to appropriate tool names
          const toolNameMap: Record<string, string> = {
            rss: 'import_rss',
            wordpress: 'import_wordpress', 
            website: 'smart_crawl',
            notion: 'migrate_notion',
            roam: 'migrate_roam',
            bear: 'migrate_bear',
            evernote: 'migrate_evernote',
            logseq: 'migrate_logseq'
          };
          
          const toolArgs = subCommand === 'rss' 
            ? { feedUrl: args.trim() }
            : subCommand === 'website'
            ? { url: args.trim(), autoDetect: true, extractImages: true, followLinks: true, maxDepth: 3 }
            : { source: args.trim() };
          
          return {
            shouldScheduleTool: true,
            toolName: toolNameMap[subCommand],
            toolArgs
          };
        }
      },
      ...
      },
      {
        ...
        action: (mainCommand, subCommand, args) => {
          ...
          if (!args?.trim()) {
            addMessage({
              type: MessageType.ERROR,
              content: `Source path/URL required for ${subCommand} import`,
              timestamp: new Date(),
            });
            return;
          }
          
          // Map import types to tool names
          const toolNameMap: Record<string, string> = {
            rss: 'import_rss',
            atom: 'import_rss', // RSS tool handles both RSS and Atom
            wordpress: 'import_wordpress',
            json: 'migrate_generic',
            markdown: 'migrate_markdown'
          };
          
          const toolArgs = subCommand === 'rss' || subCommand === 'atom'
            ? { feedUrl: args.trim() }
            : subCommand === 'json'
            ? { type: 'generic', source: args.trim() }
            : { source: args.trim() };
      ...
      },
      {
        ...
        completion: async () => ['netlify', 'vercel', 'github', 'ftp', 'obsidian-publish', 'preview'],
        action: (mainCommand, subCommand, args) => {
          if (!subCommand) {
            addMessage({
              type: MessageType.INFO,
...
Options can be passed as JSON: /deploy netlify {"domain": "example.com"}`,
              timestamp: new Date(),
            });
            return;
          }
          
          const supportedTargets = ['netlify', 'vercel', 'github', 'ftp', 'obsidian-publish', 'preview'];
          
          if (!supportedTargets.includes(subCommand)) {
            addMessage({
              type: MessageType.ERROR,
              content: `Unknown deployment target: ${subCommand}. Available: ${supportedTargets.join(', ')}`,
              timestamp: new Date(),
            });
            return;
          }
          ...
          if (args?.trim()) {
            try {
              options = JSON.parse(args.trim());
            } catch {
              // If not valid JSON, treat as simple domain option
              options = { domain: args.trim() };
            }
          }
