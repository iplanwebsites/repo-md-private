  ...
  *getConnections(tag) {
    if (!tag) {
      yield* this.#connections.values().filter(
        (c2) => c2.readyState === WebSocket.READY_STATE_OPEN
      );
      return;
    }
...
var Cron = class {
  ...
  constructor({ seconds, minutes, hours, days, months, weekdays }) {
    ...
    const validateData = /* @__PURE__ */ __name((name, data, constraint) => {
      if (data.some((x2) => typeof x2 !== "number" || x2 % 1 !== 0 || x2 < constraint.min || x2 > constraint.max)) {
    ...
    this.reversed = {
      seconds: this.seconds.map((x2) => x2).reverse(),
      minutes: this.minutes.map((x2) => x2).reverse(),
      hours: this.hours.map((x2) => x2).reverse(),
      days: this.days.map((x2) => x2).reverse(),
      months: this.months.map((x2) => x2).reverse(),
      weekdays: this.weekdays.map((x2) => x2).reverse()
    };
  }
  /**
  ...
  findAllowedHour(dir3, startHour) {
    return dir3 === "next" ? this.hours.find((x2) => x2 >= startHour) : this.reversed.hours.find((x2) => x2 <= startHour);
  }
  /**
  ...
  findAllowedMinute(dir3, startMinute) {
    return dir3 === "next" ? this.minutes.find((x2) => x2 >= startMinute) : this.reversed.minutes.find((x2) => x2 <= startMinute);
  }
  /**
  ...
  findAllowedSecond(dir3, startSecond) {
    return dir3 === "next" ? this.seconds.find((x2) => x2 > startSecond) : this.reversed.seconds.find((x2) => x2 < startSecond);
  }
  /**
  ...
  findAllowedDayInMonth(dir3, year, month, startDay) {
    ...
    if (daysRestricted) {
      allowedDayByDays = dir3 === "next" ? this.days.find((x2) => x2 >= startDay) : this.reversed.days.find((x2) => x2 <= startDay);
    ...
    if (weekdaysRestricted) {
      ...
      const nearestAllowedWeekday = dir3 === "next" ? (_a = this.weekdays.find((x2) => x2 >= startWeekday)) !== null && _a !== void 0 ? _a : this.weekdays[0] : (_b = this.reversed.weekdays.find((x2) => x2 <= startWeekday)) !== null && _b !== void 0 ? _b : this.reversed.weekdays[0];
  ...
  getNextDate(startDate = /* @__PURE__ */ new Date()) {
    ...
    let startIndexMonth = this.months.findIndex((x2) => x2 >= startDateElements.month);
  ...
  *getNextDatesIterator(startDate, endDate) {
  ...
  getPrevDate(startDate = /* @__PURE__ */ new Date()) {
    ...
    let startIndexMonth = this.reversed.months.findIndex((x2) => x2 <= startDateElements.month);
    if (startIndexMonth === -1) {
      startIndexMonth = 0;
      maxYear--;
    }
...
function parseElement(element, constraint) {
  ...
  if (listElements.length > 1) {
    for (const listElement of listElements) {
      ...
      for (const x2 of parsedListElement) {
        result.add(x2);
      }
    }
    return result;
...
function parseCronExpression(cronExpression) {
  ...
  return new Cron({
    ...
    months: new Set(Array.from(parseElement(rawMonths, monthConstraint)).map((x2) => x2 - 1)),
    weekdays: new Set(Array.from(parseElement(rawWeekdays, weekdayConstraint)).map((x2) => x2 % 7))
  });
}
...
// node_modules/repo-md/repo-md.js
var d = "/_repo/medias/";
var R = true;
function y(s) {
  const e = s.split(".").pop().toLowerCase();
  return console.log("++++++ext from path:", e, s), {
    // Images
    jpg: "image/jpeg",
    jpeg: "image/jpeg",
    png: "image/png",
...
function v(s, e) {
  ...
  let o = s.get("Content-Type");
  return console.log("++++++Content-Type from original headers:", o), (!o || o === "application/octet-stream") && (o = y(e)), t.set("Content-Type", o), console.log(
    "++++-+-+-+-+++Content-Type from findProbableMimeType",
    o
  ), t.delete("Content-Disposition"), t.set("Cache-Control", "public, max-age=333"), t;
}
__name(v, "v");
async function w(s, e) {
  console.log(`[MediaProxy] Handling request: ${s.url}`);
  const t = new URL(s.url), o = t.pathname.startsWith(d);
  if (console.log(`[MediaProxy] URL path: ${t.pathname}, isMedia: ${o}`), !o)
    return null;
  console.log(
    "[MediaProxy] Detected media request, proxying to asset server"
  );
  const i = t.pathname.replace(d, "");
  console.log(
    `[MediaProxy] Extracted media path: ${i} from ${t.pathname}`
  );
  const n = await e(i);
  console.log(`[MediaProxy] Proxying to R2 URL: ${n}`);
  ...
  try {
    ...
    R && console.log(`[MediaProxy] R2 response status: ${a.status}`);
    const g = v(a.headers, i);
    ...
    return console.error("[MediaProxy] Error proxying to asset server:", a), new Response("Asset not found", { status: 404 });
  }
}
__name(w, "w");
var p = "/_repo/medias";
var c = "https://static.repo.md";
var x = {
  [p]: {
    target: c,
    changeOrigin: true
...
var l = new QuickLRU({
  maxSize: 1e3,
  ///tweak depending on worker
  maxAge: 6e4 * 60
  // 1h
});
async function U(s, e = {}, t = false) {
  const {
    errorMessage: o = "Error fetching data",
    defaultValue: i = null,
    useCache: n = true
  } = e;
  try {
    if (t && console.log(`[RepoMD] Fetching JSON from: ${s}`), n && l && l.has(s)) {
      const g = l.get(s);
      return t && console.log(`[RepoMD] Cache hit for: ${s}`), g;
    }
    const r = await fetch(s);
    if (!r.ok)
      throw new Error(`${o}: ${r.statusText}`);
    const a = await r.json();
    return n && l && (l.set(s, a), t && console.log(
      `[RepoMD] Cached data for: ${s} (cache size: ${l.size})`
    )), a;
  } catch (r) {
    return t && (console.error(`[RepoMD] \u{1F310}  ${s}:`), console.error(`[RepoMD] ${o}:`, r)), i;
  }
}
__name(U, "U");
var A = "https://r2.repo.md";
var S = class {
  static {
    __name(this, "S");
  }
  constructor({
    org: e = "iplanwebsites",
    orgSlug: t = "iplanwebsites",
    orgId: o = null,
    project: i = "tbd",
    projectId: n = "680e97604a0559a192640d2c",
    projectSlug: r = "undefined-project-slug",
    rev: a = "latest",
    // Default to "latest"
    secret: g = null,
    debug: f = false
    ...
    this.org = e, this.project = i, this.projectId = n, this.projectSlug = r, this.orgSlug = t, this.orgId = o, this.rev = a, this.debug = f, this.secret = g, this.activeRev = null, this.debug && console.log(`[RepoMD] Initialized with:
        - org: ${e}
        - project: ${i}
        - rev: ${a} 
        `);
  ...
  getR2Url(e = "") {
    return this.getR2RevUrl(e);
  }
  getR2ProjectUrl(e = "") {
    const t = `${A}/${this.orgSlug}/${this.projectId}${e}`;
    return this.debug && console.log(`[RepoMD] Generated URL: ${t}`), t;
  }
  getR2RevUrl(e = "") {
    const t = this.rev === "latest" ? this.activeRev : this.rev, o = this.getR2ProjectUrl("/" + t + e);
    return this.debug && console.log(`[RepoMD] Generated URL: ${o}`), o;
  }
  // Get base API URL for backend calls
  async fetchPublicApi(e = "/") {
    const o = `https://api.repo.md/v1/${e}`, { success: i, error: n, data: r } = await this.fetchJson(o, {
      errorMessage: "Error fetching pubic API route: " + e,
      useCache: true
      // fetchJson already handles caching
    });
    return r;
  }
  // Fetch project configuration including latest release information
  async fetchProjectDetails() {
    const e = `/orgs/${this.orgSlug}/projects/slug/${this.projectSlug}`;
    return await this.fetchPublicApi(e);
  }
  // Get the latest revision ID
  async getActiveProjectRev() {
    const { activeRev: e, id: t } = await this.fetchProjectDetails();
    return e;
  }
  // Ensure latest revision is resolved before making R2 calls
  async ensureLatestRev() {
    if (this.rev === "latest" && !this.activeRev) {
      const e = await this.getActiveProjectRev();
      if (!e)
        throw new Error("Could not determine latest revision ID");
      this.activeRev = e, this.debug && console.log(
  ...
  async getR2MediaUrl(e) {
    return this.getR2ProjectUrl(`/_shared/medias/${e}`);
  }
  // Fetch a JSON file from R2 storage
  async fetchR2Json(e, t = {}) {
    await this.ensureLatestRev();
    const o = this.getR2Url(e);
    return await this.fetchJson(o, t);
  }
  // Fetch all blog posts
  async getAllPosts(e = true) {
  ...
  // Fetch media data
  async getAllMedia(e = true) {
    const t = await this.fetchR2Json("/medias.json", {
      defaultValue: {},
      useCache: e
    });
    return this.debug && console.log("[RepoMD] Fetched media data:", t), t;
  }
  // Get all media items with formatted URLs
  async getMediaItems(e = true) {
    const t = await this.getAllMedia(e);
  ...
  async getPostById(e) {
    this.debug && console.log(`[RepoMD] Fetching post with ID: ${e}`);
    const t = await this.getAllPosts();
    return (t == null ? void 0 : t.find((o) => o.id === e)) || null;
  }
  // Get a single blog post by slug
  async getPostBySlug(e) {
    this.debug && console.log(`[RepoMD] Fetching post with slug: ${e}`);
    const t = await this.getAllPosts();
    return (t == null ? void 0 : t.find((o) => o.slug === e)) || null;
  }
  // Sort posts by date (newest first)
  sortPostsByDate(e) {
    return [...e].sort((t, o) => new Date(o.date) - new Date(t.date));
  }
  // Get recent posts
  async getRecentPosts(e = 3) {
    const t = await this.getAllPosts();
    return this.sortPostsByDate(t).slice(0, e);
  }
  // Get release information
  async getReleaseInfo() {
    const e = await this.fetchProjectDetails();
    return {
      current: e.latest_release,
      all: e.releases || []
    };
  }
  // Handle Cloudflare requests
  async handleCloudflareRequest(e) {
    return this.debug && console.log(`[RepoMD] Handling Cloudflare request: ${e.url}`), await w(e, async (o) => await this.getR2MediaUrl(o));
  }
};

// src/index.ts
var MyMCP = class extends McpAgent {
  constructor({ org, projectId }) {
    super();
    this.server = new McpServer({
      name: "Repo.md MCP Server",
      version: "0.0.3"
    });
    this.repoClient = new S({
      orgSlug: org,
      projectId,
      debug: true
    });
        ...
        try {
          ...
          return {
            content: [
              {
                type: "text",
                text: `Error fetching blog posts: ${error3 instanceof Error ? error3.message : String(error3)}`
              }
            ]
          };
        }
...
var app = new Hono2();
app.get("/org/:org/projects/id/:projectId/sse", async (c2) => {
  const { org, projectId } = c2.req.param();
  return MyMCP.serveSSE(`/org/${org}/projects/id/${projectId}/sse`, { org, projectId }).fetch(c2.req.raw, c2.env, c2.executionCtx);
});
app.get("/org/:org/projects/id/:projectId/sse/message", async (c2) => {
  const { org, projectId } = c2.req.param();
  return MyMCP.serveSSE(`/org/${org}/projects/id/${projectId}/sse`, { org, projectId }).fetch(c2.req.raw, c2.env, c2.executionCtx);
});
app.get("/sse", async (c2) => {
  return MyMCP.serveSSE("/sse").fetch(c2.req.raw, c2.env, c2.executionCtx);
});
app.get("/sse/message", async (c2) => {
  return MyMCP.serveSSE("/sse").fetch(c2.req.raw, c2.env, c2.executionCtx);
});
app.get("/mcp", async (c2) => {
  return MyMCP.serve("/mcp").fetch(c2.req.raw, c2.env, c2.executionCtx);
});
app.notFound((c2) => {
...
// .wrangler/tmp/bundle-lIcRBa/middleware-insertion-facade.js
...
// .wrangler/tmp/bundle-lIcRBa/middleware-loader.entry.ts
